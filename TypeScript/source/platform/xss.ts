...
export class Xss {
  ...
  public static htmlSanitizeKeepBasicTags = (dirtyHtml: string): string => {
      ...
      transformTags: {
        'img': (tagName, attribs) => {
          ...
          if (srcBegin.indexOf('data:') === 0) {
            ...
            return { tagName: 'a', attribs: { href: String(attribs.src), target: "_blank" }, text: '[remote image]' };
          } else {
            return { tagName: 'span', attribs: {}, text: '[remote image]' } as Tag;
          }
        }
      },
    });
    if (transformed) {
      ...
      cleanHtml = dereq_html_sanitize(cleanHtml, { allowedTags: Xss.ALLOWED_BASIC_TAGS, allowedAttributes: Xss.ALLOWED_ATTRS });
    }
    return cleanHtml;
  }
