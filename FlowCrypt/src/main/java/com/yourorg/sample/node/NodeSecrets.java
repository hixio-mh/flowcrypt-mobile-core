...
public class NodeSecrets {
  ...
  public NodeSecrets() throws Exception {
    generate();
  }
  ...
  private void generate() throws Exception {
    ...
    // finalize
    this.ca = crtToString(caCrt);
    this.crt = crtToString(srvCrt);
    this.key = keyToString(srvKeypair.getPrivate());
    this.sslContext = sslContext;
    this.authPwd = genPwd();
    this.authHeader = "Basic " + DatatypeConverter.printBase64Binary(this.authPwd.getBytes());
  }

  private KeyStore newKeyStore(String alias, Certificate crt, KeyPair keyPair) throws Exception {
